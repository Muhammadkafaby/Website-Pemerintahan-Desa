...
const form = useForm({
    _method: 'PUT',
    name: props.government.name,
    position: props.government.position,
    photo: null,
    history: props.government.history,
});

const submit = () => {
    form.post(route('admin.governments.update', props.government.id));
};
</script>

<template>
    <Head title="Edit Aparatur Desa" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Aparatur Desa</h2>
        </template>
